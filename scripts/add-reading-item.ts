...
type ContentType = 'resource' | 'video' | 'article' | 'tweet' | 'tool';

const isValidType = (type: string): type is ContentType => {
  return ['resource', 'video', 'article', 'tweet', 'tool'].includes(type);
};
...
async function main() {
  try {
    ...
    const currentData: ReadingData = eval(`(${match[1]})`);
